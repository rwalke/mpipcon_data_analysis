...
p1MonthFacet <- ggplot(power3[year3<2024], aes(x=factor(month3), y=power1)) + geom_boxplot(fill="lightblue",outlier.size = 1.0) + facet_wrap(~year3) +
  scale_x_discrete("month") + scale_y_continuous("power consumption in kW")
p1MonthFacet

p1MonthFill <- ggplot(power3[year3<2024], aes(x=factor(month3), y=power1, fill=factor(year3))) + geom_boxplot(outlier.size = 1.0) +
  scale_x_discrete("month") + scale_y_continuous("power consumption in kW") + scale_fill_discrete("year")
p1MonthFill

p1WeekFill <- ggplot(power3[year3<2024], aes(x=factor(week3), y=power1, fill=factor(year3))) + geom_boxplot(outlier.size = 1.0) +
